...
try:
    ...
    print("A biblioteca Pillow é necessária. Instale com: pip install Pillow")
    sys.exit()
...
class CronometroPremium:
    ...
    def manter_sempre_visivel(self):
        ...
        while self.topmost_thread_running:
            try:
                if self.config.get("sempre_visivel", True) and self.root.winfo_exists() and not self.is_break_time:
                    self.root.attributes('-topmost', True)
                    self.root.lift()
